...
namespace GXPEngine
{
	...
	public abstract class GameObject : Transformable
	{
		...
		// moves towards a Vector2 at a given speed
		public bool moveTowards(Vector2 target, int speed, int stopDistance = 5)
		{
            movement = movement.subVectors(target, new Vector2(x, y));

            movement = movement.setMagnetude(movement, speed);

            x += movement.x;
            y += movement.y;

            if (movement.distance(new Vector2(x, y), target) < stopDistance)
            {
				return true;
            }
