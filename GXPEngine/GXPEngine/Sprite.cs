...
namespace GXPEngine
{
	...
	public class Sprite : GameObject
	{
		...
		protected Vector2 _textureOffset = new Vector2(0, 0);
		...
		/// If <c>true</c>, this sprite will have a collider that will be added to the collision manager.
		/// </param> 
		public Sprite (string filename, bool keepInCache=false, bool addCollider=true, bool isTextureOffset = true) : base(addCollider)
		{
			if (Game.main == null) {
				throw new Exception ("Sprites cannot be created before creating a Game instance.");
			}
			name = filename;
			initializeFromTexture(Texture2D.GetInstance(filename, keepInCache), isTextureOffset);
		}

		//------------------------------------------------------------------------------------------------------------------------
		//														initializeFromTexture()
		//------------------------------------------------------------------------------------------------------------------------
		protected void initializeFromTexture (Texture2D texture, bool isTextureOffset = true) {
			_texture = texture;
            if (isTextureOffset)
            {
                _bounds = new Rectangle(-_texture.width/2, -_texture.height/2, _texture.width, _texture.height);
            }
			else
			{
